...
// Marker object for ad placeholder
object AdPlaceholder

class JinjaSoapCardAdapter(
    private var items: List<Any>, // Accepts both ADModel and AdPlaceholder
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        return if (viewType == VIEW_TYPE_AD) {
            val view = LayoutInflater.from(parent.context)
                .inflate(R.layout.ad_mob_soap_item, parent, false)
            AdViewHolder(view)
        } else {
            val view = LayoutInflater.from(parent.context)
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        if (holder is AdViewHolder) {
            ...
            ADViewModel(adRepository).fetchUserDetails(card.posterId) { _, _, profileImage, _ ->
    ...
    // Use DiffUtil to update items efficiently
    ...
    // ViewHolder for content (soap ad) items
